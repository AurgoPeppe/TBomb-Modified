...
try:
    ...
    print('[!] Errore: alcune dipendenze non sono installate')
    print('Scrivi \'pip install -r requirements.txt\' per installare tutti i pacchetti richiesti')
    exit()
...
def checkinternet():
    ...
    try:
        ...
        res = True
    if res:
        print("\n\n\tSembra che la tua velocità di Internet sia lenta o che tu stia utilizzando i proxy ...")
        print('\t\tL\' attacco sta per essere avviato...\n\n')
        banner()
        exit()
...
def start(target, counter, delay, ch, cc):
    ...
    while success < (int(bombs)):
        ...
        try:
            ...
            if cc == "91":
                print('Scusate tutte le API Non sono funzionanti')
                input('Premi INVIO per usicre...')
                exit()
            else:
                if success > 0:
                    print(
                        '\n\n\tSiamo spiacenti di affermare che è stato raggiunto il limite di bombardamenti per il tuo Paese...')
                    print(
                        '\nStiamo lavorando troppo duramente per aumentare il limite internazionale ...')
                    input(
                        '\nQuesto ci aiuterà a fornire rapidamente supporto al tuo Paese ... \n\ nPremi Invio per uscire...')
                    os.system('rm *.xxx* > /dev/null 2>&1')
                    print('\n\n')
                    banner()
                    exit()
                else:
                    print('\n\n\tScusaci il tuo Paese non è supportato...')
                    print(
                        '\t\Perfavore invia l\' email a alexzfortnitez23@gmail.com..')
                    input('Premi invio per uscire...')
                    exit()
        print("==================================================================")
        print("                ATTACCO IN CORSO, PERFAVORE ASPETTA               ")
        print("     Perfavore resta online mentre un attacco è in corso          ")
        print("==================================================================")
        print("             Numero Bersaglio           : +" + str(cc) + " ", target)
        print("             Numero richieste inviate : ", requested)
        print("             Richieste Confermate     : ", success)
        print("             Richieste Fallite         : ", failed)
        print("==================================================================")
        print("         Attenzione, usa il programma per divertimento !!         ")
        print("             Il programma è stato modificato da Ales!             ")
        print("==================================================================")
